...
- **Per-mesh primitive ceiling — raised, not worked around** (verified). A district mesh draws as *sub-particles*
  ...
  **How far the ceiling goes (2026-09-13 investigation, IL of `GetEncodedMeshAndVisualParticleCount` read from
  ...
  `255 × PPC × DistrictMeshDensityBoost`, raise the config int as needed (default 8 ≈ 130k prims; 32 ≈ 520k —
  the scoped path's low-PPC donor needed exactly that). The REAL remaining walls are: (1) the shared **Visual
