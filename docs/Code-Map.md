...
Where things live in HAF. The repository contains both the BepInEx runtime and the installable Unity editor package;
ENCReload is a consumer and reference pack, not the editor source.
...
- Adding a **registry field**: the fields stored identically by both classes are the **shared
  ...
  `Regex.Matches` line there. **Then run the drift guard:** `bash tools/check_schema_parity.sh` — it fails
