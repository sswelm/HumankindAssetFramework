...
- [**Turn-Ease.md**](Turn-Ease.md) — **turn first, aim true, fire second**: eased facing for any HAF model **and vanilla units** (Formation Lab links), the map-bombard hold that keeps every effect of the shot waiting on one clock, and the true-bearing aim that replaces vanilla's hex-quantized artillery facing. Includes the choreography-seam map + graveyard for maintainers.
