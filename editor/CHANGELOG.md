...
- **Vehicle Lab: a Window role with its own reduce dial** (user: "add the reducable part type Window"). Glazing -
  ...
  `windowreduce=@file|percent`; an older script ignores the tag, so it and a newer Lab still run together; a
  malformed tag is a hard error. **Review:** the Window and Shroud names files are written and sent at every dial
  setting (percent 0 when the dial is off), not only above 0 % - the script's artifact purge protects a marked part
  only when its name arrives, and a sphere-shaped pane still called Icosphere was deletable at 0 % despite the mark. Test: the tag's names text and percent, absent at 0, clamped 0..95, a '|' inside a
