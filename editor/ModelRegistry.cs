...
public class ModelDef : Haf.Schema.HafModelSchema
{
    ...
    public bool staticAxisV2 = false;   // STATIC only (BAKE-TIME, glb/gltf sources; 2026-09-16 axis unification): glbconv converts Y-up->Z-up and the combine skips the longest-axis auto-align, so the static frame — and the Rotation field's meaning — MATCHES the animated path (the agreed canon; the legacy static frame arrived pitched 90°, hand-compensated per entry). The FACTORY sets it true on NEW entries; the initializer stays false so every pre-v2 entry (absent key) re-bakes byte-identically with its compensated Rotation.
