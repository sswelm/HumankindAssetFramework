...
public static class WorkshopRules
{
    ...
    // THE SIDECAR OF A FUSED OUTPUT (review of PR #63: "the Fuser -> Cutter handoff loses group letters"): every group that
    // fused is now ONE shell, and the shell carries the group's letter under its own node index; a group that produced
    // nothing keeps its parts' lines unchanged (the fuse never renumbers nodes). So a fused output opened in the Cutter
